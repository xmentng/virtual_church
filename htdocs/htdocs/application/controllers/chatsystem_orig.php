...
function savepost(){
	...
	if($ddd != ""){
		...
		$isposted = mysql::insert(array("message"=>$ddd, "sender"=>$page_res['logged_in_account'], "receiver"=>$page_res['church_account_name'],"recd"=>1, 'time_posted'=>misc::serverTime()), "tbl_chat_messages");
...
function refreshpost(){
	...
	//echo $today(); exit(0);
	$result = mysql_query("SELECT * FROM tbl_chat_messages WHERE sender=\"$page_res[logged_in_account]\" ORDER BY id DESC LIMIT 10");
...
function chatHeartbeat() {
	...
	$sql = "select tbl_chat_users.username,tbl_chat.from,tbl_chat.message,tbl_chat.to,tbl_chat.id,tbl_chat.sent,tbl_chat.recd from tbl_chat,tbl_chat_users where (tbl_chat.to = '".mysql_real_escape_string($page_res['logged_in_account'])."' AND recd = 0) and tbl_chat.from=tbl_chat_users.username order by id ASC";
...
EOD;
	...
	$sql = "update tbl_chat set recd = 1 where chat.to = '".mysql_real_escape_string($page_res['church_account_name'])."' and recd = 0";
	$query = mysql_query($sql);
...
?>
{
		"username": "<?php echo $page_res['logged_in_account'];?>",
		"items": [
			<?php echo $items;?>
        ]
}
...
$su=$_GET['usw'];
$sc2=mysql_query("select username from tbl_chat_users where username='$su' limit 1");
...
function sendChat() {
	...
	$from = $page_res['logged_in_account'];
	$to = $_POST['to'];
	
	$message = $_POST['message'];
	$sql = "select tbl_chat_users.username from tbl_chat_users where tbl_chat_users.username='$from' limit 1";
...
EOD;
	...
	$sql = "insert into tbl_chat (tbl_chat.from,tbl_chat.to,message,sent) values ('".mysql_real_escape_string($from)."', '".mysql_real_escape_string($to)."','".mysql_real_escape_string($message)."',NOW())";
	$query = mysql_query($sql);
	echo "1";
	exit(0);
}
