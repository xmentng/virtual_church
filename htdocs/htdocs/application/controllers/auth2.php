...
function processlogindetails(){
	...
	if($user){
		...
		if($_acc_level){
				...
				mysql::update($tbl = 'tbl_users', $setflds = array('is_logged_in'=>1), $where=array('id'=>$this->session->userdata('user_id')));
...
function logout(){
	...
	mysql::update($tbl = 'tbl_users', $setflds = array('is_logged_in'=>0), $where=array('id'=>$this->session->userdata('user_id')));
