                ...
                <div class="nine columns">
                    ...
                    <p style="line-height:1.8125em; text-align:justify; font-size:11.8px;">
                      ...
                      <form method="post" name="frmreport" id="frmreport">
                            ...
                            <code id="post-result-msg" style="padding:0px 7px;"><?php echo $data['info_msg']  ?></code>
                      		<div class="twelve colums">
                            	church service date: 
                                <input name="service_date" type="text" id="datepicker"  class="cls_txtflds" style="width:45%;" />
                                <br>
								<input name="cmdgo" id="cmdgo" type="submit" value="&nbsp;&nbsp;Submit&nbsp;&nbsp;" class="btn pretty primary" style="color:#FFF;">
                            </div>
                            <?php
								...
								//if($data['posted']){
									...
									if(@$data['nattendees'] > 0){
										$sn =0;
									
							?>
                            <div class="row">
                            	<div class="twelve columns" style="" id="chart-position">
...
$('#cmdgo').click(function(){
	...
	$('#post-result-msg').removeClass('success');
	$('#post-result-msg').removeClass('error');
	$('#post-result-msg').addClass('info');
	$('#post-result-msg').html('<img src="/images/loading.gif"  align="absmiddle" />&nbsp;Please wait...');
	
	
	$.ajax({
			 type: "POST",
				   url:	"/churchadmin/getChurchAttendaceSummaryChatReport",
				   ...
				   success:	function(e){
					   alert(e);return false;
						var sp = e.split('|');
							...
							if(sp[0] == "success"){
								//alert(e);return false;
								$('#post-result-msg').addClass("success");
								$('#post-result-msg').removeClass("error");
								
								$('#post-result-msg').html( sp[1]);
								$('#frmadd')[0].reset();

								get_hmenu_html_value();
								//$('#loader_pos').hide();
							...
							if(sp[0] == "failure"){
								...
								$('#post-result-msg').removeClass('success');
								$('#post-result-msg').addClass('error');
								$('#post-result-msg').html(sp[1]);
