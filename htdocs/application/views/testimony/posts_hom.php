	...
	if(checkValues($_REQUEST['value']))
	{
		...
		echo "INSERT INTO testimony_posts (post,f_name,userip,date_created,user_id) VALUES('".checkValues($_REQUEST['value'])."','".$name."','".$userip."','".strtotime(date("Y-m-d H:i:s"))."','".$id."')";
		...
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM testimony_posts,users where testimony_posts.user_id =users.id  order by p_id desc limit 1");
	...
	elseif($_REQUEST['show_more_post']) // more posting paging
	{
		...
		$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM testimony_posts,users where testimony_posts.user_id =users.id order by p_id desc limit ".$_REQUEST['show_more_post'].", 4");
		
		$check_res = mysql_query("SELECT * FROM testimony_posts,users where testimony_posts.user_id =users.id order by p_id desc limit ".$next_records.", 4");
	...
	{	
		$show_more_button = 1;
		$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM testimony_posts,users where testimony_posts.user_id =users.id  order by p_id desc limit 0,4");
...
?>
	   <img src="<?php echo "/picdisplay.php?dm=".base64_encode('40X40').'&pt='.base64_encode("pics/$pics"); ?>" style="float:left;" alt="<?php echo "$row[Fist_Name] $row[Last_Name]";?>" />
		   ...
		   $net_vote = $row['votes_up'] - $row['votes_down'];?>
		   	<span class='votes_count' id='votes_count<?php echo $row['p_id']; ?>'><?php echo $net_vote." likes"; ?></span>
            <?php
			 $p_id =$row['p_id'];
			 $userip = $_SERVER['REMOTE_ADDR'];
...
$pics1 = $rows['pics'];
?>
			<img src=" <?php echo "/picdisplay.php?dm=".base64_encode('30X30').'&pt='.base64_encode("pics/$pics1"); ?>"  style="float:left;" alt="<?php echo "$row[Fist_Name] $row[Last_Name]";?>"class="CommentImg"  />
<strong style=" text-align:left; margin-left:5px; float:left; color:#036 ;font-weight:bold;"> <?php echo "$row[Fist_Name] $row[Last_Name]";?></strong>
