...
<!--main content top-->
 ...
 <div class="row">
 		<div class="twelve columns" style="font-size:0.875em;">
                ...
                <form name="this-form" id="this-form" method="post">
                    <div class="row" style="background-color:#70819C; color:#FFF;">
                    	<div class="twelve columns">
                    ...
                    <?php
						$nrecords = count($meeting_info['id']);
						if($nrecords > 0){
							...
							$end_time = intval($meeting_info['meeting_time'][$i] + $meeting_info['meeting_duration'][$i]);
					?>
                    <div class="row" style="border-bottom:solid 1px #70819C; margin-bottom:2px; margin-top:2px;">
                    	<div class="twelve columns">
							<!--<div class="two columns">
                            ...
                            <div class="three columns">
                            	<span>
									<?php  if($meeting_info['meeting_title'][$i])echo $meeting_info['meeting_title'][$i];   ?>
                                    <?php  if(!$meeting_info['meeting_title'][$i])echo "NIL";   ?>
                                 </span>
                            </div>
                            
                            <div class="three columns">
                            	<span><?php  echo date("F j,Y g:i a", $meeting_info['meeting_time'][$i]);   ?></span>
                            </div>
      
                            <div class="three columns">
                            	<span><?php  echo date("F j,Y g:i a", $end_time);   ?></span>
                            </div>
                            
                            <div class="two columns">
                            <!--<a href="javascript:void(0)" id="meeting_<?php //echo $meeting_info['id'][$i]  ?>" class="cls_edit  btn">
                            <span style="padding:0px 7px;">Edit</span></a>-->
                               
                            <a href="javascript:void(0)" id="<?php echo $meeting_info['id'][$i]  ?>" class="cls_cancel  btn">
                            <span style="padding:0px 7px;">Cancel</span></a>
                            </div>
                         </div>   
					</div>
                    <?php
...
function cancel_meeting(param){
	
	//alert(param);
	
	$.ajax({
		   type: "POST",
		   url:		"/delete/cancel_ref_meeting/" + param,
		   data: $('#this-form').serialize(),
		   success:	function(e){
			   alert(e);
			   document.location="/churchadmin/cellmeeting/view";
...
$('.cls_cancel').click(function(){
	...
	if (confirm("Do you want to cancel this meeting?")) {
        cancel_meeting(id);
    }
    return false;
});
