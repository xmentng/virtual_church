    ...
    <ul>
        ...
        <li class="field">
        	...
        	<select id="cell_id" name="cell_id"  class="text input" required>
                <option selected="selected" value="<?php echo $cell_leader['cell_id'][0];  ?>"><?php  echo $page_res['cell_name'];  ?></option>
                <?php
					if($data['n_cells'] > 0 ){
        ...
        <li class="field"><input class="text input" type="text" placeholder="Cell Leader E-Mail" name="cleader_email" id="cleader_email" required value="<?php echo $cell_leader['cell_leader_email'][0];  ?>" /></li>
        <li class="field"><input class="text input" type="text" placeholder="Cell Leader Name" name="cleader_name" id="cleader_name" required value="<?php echo $cell_leader['cell_leader_name'][0];  ?>" /></li>
        <li class="field">
        	<select id="country" name="country"  class="text input" required>
                <option selected="selected" value="<?php echo $cell_leader['country'][0];  ?>">---country---</option>
        ...
        <li class="field"><input name="church_id" type="hidden" value="<?php echo @$page_res['church_id'][0]; ?>">
        <input name="id" type="hidden" value="<?php echo @$cell_leader['id'][0]; ?>"></li>
       
        <li class="field">
        	<input name="submit_btn" id="cmdclick" class="pretty  primary btn" type="submit" value="&nbsp; Update Cell Leader &nbsp;" style="color:#FFF; text-shadow:1px 1px 1px #333; width:auto; padding:2px 7px;"></li>
    </ul>
    </form>	
...
$('#cmdclick').click(function(){
		...
		$.ajax({
			   type: "POST",
			   		url:	"/postmanager/update_cell_leader",
					data:	$('#this-form').serialize(),
					success:	function(e){
