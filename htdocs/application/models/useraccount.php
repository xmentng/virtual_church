...
function isUnique($tblname, $data){
	$query = $this->db->get_where($tblname, $data);
...
function getUserLoginDetails($sql){
  ...
  if(mysql::size($resid) > 0){
	...
	function has_prayed($user_id, $prayer_id){
		$sql = "SELECT * FROM i_prayed WHERE user_id='".$user_id."' AND prayer_id='".$prayer_id."'";
		$query = $this->mysql->query($sql);
		return ($this->mysql->size($query) > 0)?true:false;
	}
	
	function say_prayed($user_id, $prayer_id){
		$sql = "INSERT INTO i_prayed(user_id, prayer_id) VALUES('{$user_id}', '{$prayer_id}')";
		$query = $this->mysql->query($sql);
		return ($query) ? true : false;
	}
	
	function say_not_prayed($user_id, $prayer_id){
		$sql = "DELETE FROM i_prayed WHERE user_id='".$user_id."' AND prayer_id='".$prayer_id."'";
		$query = $this->mysql->query($sql);
		return ($query) ? true : false;
	}
...
function redirect_not_cell_leader(){
 ...
 function redirectuser(){
			...
			#lets check if this user is an admin / super user
				...
				$sql = "SELECT * FROM tbl_users WHERE user_name=\"$logged_in_account\" AND status='1' AND enabled='1' LIMIT 1";
...
function _isAccountConfigured($userName){
          ...
          $sql = "SELECT enabled FROM tbl_users WHERE user_name='$userName' LIMIT 1";
          //run the query
          $res = $this->mysql->query($sql);
          $row = $this->mysql->fetch($res);
		  echo $row['enabled']; exit;
          if($row['enabled'] == '1'){
              return true;
          }
          else{
...
function _updateUserAttributes($whereAttr, $setAttr){
	$is_updated = querymanager::update("tbl_users", $setAttr, $whereAttr);
...
function dispatchActivationMail($detail, $tblname){
 ...
 function loadDetails($tableName,$arrFilter,$arrAttribute,$num=NULL,$orderBy=''){
          ...
          $limitStr='';
           ...
           $sql = "SELECT [FIELDS] FROM ".$tableName.$whereStr.$orderByStr.$limitStr;
          // echo $sql;  
