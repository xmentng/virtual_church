...
class churchadmin extends CI_Controller {
	...
	function general_page_resource(){
		...
		$church_banner = useraccount::getLastAttributeValue(array('id', 'church_banner_url'), $tblname='tbl_churches', array('id'=>$church_id), $retval='church_banner_url');
	...
	function uploadbanner(){
		...
		$savePath = CUSTOM_BANNER_PATH.$this->misc->genRand(mt_rand(5,10)).".".$pathInfo['extension'];
		...
		// generate thumbnail of 200px by 200px
		  ...
		  if(move_uploaded_file($_FILES['picture']['tmp_name'],$saveRelPath)){
			  ...
			  /////////////////////////THUMBNAIL RESIZE SCRIPT HERE/////////////////////////////////////////////////////////////
							...
							//echo $pt; exit;
							//echo json_encode(array('status'=>true, 'message'=>$pt)); exit;
							
							$dm = base64_encode("950X150");
							//echo $dim; exit;
							...
							//echo json_encode(array('status'=>true, 'message'=>$pt2)); exit;
			  ...
			  //echo json_encode(array('status'=>true, 'message'=>$page_res['church_id'])); exit;
			  
			  $this->mysql->update('tbl_churches', array('church_banner_url'=>$savePath), array('id'=>$page_res['church_id']));
			  
			 // $this->session->set_userdata('video_thumbnail_url', $saveVideoThumbnailPath);
  
			  echo json_encode(array('status'=>true,'message'=>'&nbsp;Banner successfully uploaded.')); exit;
		  }else{
	...
	function dashboard(){
		...
		$data['page_title'] = "Welcome to Christ Embassy Virtual Church";
		$page_res['page_name'] = "DASHBOARD";
		
		$this->load->view('v2/church_admin/vw_dashboard', array('data'=>$data, 'page_res'=>$page_res));
	}//end function
	...
	function get_detail_by_username($param){
		...
		$view = 'church_admin/manage_users_account';
				...
				$data['page_title'] = ":: Christ Embassy Live Streaming - Manage User Account ::";
				$data['info_msg'] = "";
				$page_res['page_name'] = "MANAGE CHURCH USER ACCOUNT";
				$data['page_desc'] = "";
				$users = array();
				...
				$account_users = useraccount::getUsersWithUserName($param);
				
				//echo $param; exit;
				//var_dump($account_users); exit;
	...
	function upload_generalpage_banner($view, $data){
		...
		if($is_uploaded):
			...
			#lets insert
			$is_added = mysql::insert($detail, 'tbl_church_banners');
			if($is_added){
				$data['flag'] = "success";
...
function invite_link(){
	...
	switch ($action):
		...
		case 'generate':
			...
			$this->session->set_userdata('invite_link',$data['invite_link']); 
...
function generic_link(){
	...
	#lets load the view 
			...
			$data['month'] = date('m');
			$data['day'] = date('d');
			$data['church_name'] = $this->session->userdata('church_name');
	...
	global $church_id, $invite_link;
					...
					while (($filedata = fgetcsv($fh, 1000, ",")) !== FALSE):
						...
						if($spss_row > 1)
						{
							...
							$detail = array("church_id"=>$page_res['church_id'],
											"msg_title"=>strip_tags($_POST['msg_title']),
											"msg_from"=>strip_tags($_POST['msg_from']),
...
function process_uploaded_excel_file($view, $file_detail, $data){
    ...
    while($x<=$rows) :
	  ...
	  if($y > $cols){
			...
			$detail['church_id'] = $page_res['church_id'];
			$detail['msg_title'] = strip_tags($_POST['msg_title']);
			$detail['msg_from'] = strip_tags($_POST['msg_from']);
...
function save_to_chat_admin_users(){
	...
	mysql::insert(array('user_account'=>$chatuser, 'admin_account'=>$page_res['church_account_name'], 'time_connected'=>$time), 'tbl_admin_and_user_chat_cn');
	
	$this->session->set_userdata(array('chat_login_time'=>$time));
...
function report(){
		...
		switch ($type):
			...
			case 'summarychurchattendance':
				
				$view = 'church_admin/reports/summary_churchattendace';
				$data['css_cls'] = "info";
				$data['page_title'] = ":: Summary Church Attendance Report ::";
				$data['info_msg'] = "<img src=\"/images/icons/info_small.png\" /> &nbsp; Kindly choose a service date.";
				$page_res['page_name'] = "<span style='font-weight:bolder;'>Report System</span>";
				
				if(isset($_POST['cmdgo'])){
					
					$data['posted'] = true;
					$this->getChurchAttendaceSummaryReport($view, $data);
			...
			case 'detailchurchattendance':
				$this->getDetailChurchAttendanceReport();
			break;
			
			case 'detailedchurchattendancereport':
			
				$view = 'church_admin/reports/detailed_churchattendace';
				$data['css_cls'] = "info";
				$data['page_title'] = ":: Detailed Church Attendance Report ::";
				$data['info_msg'] = "<img src=\"/images/icons/info_small.png\" /> &nbsp; Kindly select a service date.";
				$page_res['page_name'] = "<span style='font-weight:bolder;'>Report System</span>";
				
				if(isset($_POST['cmdgo'])){
					
					$data['posted'] = true;
					$this->getDetailedChurchAttendanceReport($view, $data);
...
function getChurchAttendaceSummaryReport($view, $data){
	
	global $page_res;
	$this->general_page_resource();
	
	$param = strip_tags($_POST['service_date']);
	
	if(misc::required($param)){
		$this->session->set_userdata(array('service_date'=>$param));
		//echo $param;
		
		$arrmonth = explode('/', $param);
		
		$month  = intval($arrmonth[0]);
		
		$day = intval($arrmonth[1]);
		
		$year = intval($arrmonth[2]);
		
		$selected_time = mktime(0,0,0,$month, $day, $year);
		
		$this->session->set_userdata(array('service_time'=>$selected_time));
	
		$arrAttendanceInfo = churchmanager::getSummaryChurchAttendanceReport($selected_time, $page_res['church_id']);
		
		
		
		$data['nattendances'] = count($arrAttendanceInfo['id']);
		
		
		//lets load the view once again
		$data['info_msg'] = "<img src='/images/icons/info_small.png\' /> &nbsp; Find below report information.";
		
		$page_res['page_name'] = "<span style='font-weight:bolder;'>Report System</span>";
		
		$this->load->view($view, array('data'=>$data, 'page_res'=>$page_res, 'attendanceInfo'=>$arrAttendanceInfo));
	
	}else{
		
		$data['info_msg'] = "<img src=\"/images/icons/info_small.png\" /> &nbsp; Kindly select a sevice date.";
...
function getDetailChurchAttendanceReport(){
	...
	$service_time = $this->session->userdata('service_time');
	
	$arrAttendanceInfo = churchmanager::getDetailChurchAttendanceReport($service_time, $church_id);

	$data['nattendances'] = count($arrAttendanceInfo['church_id']);

	$view = 'church_admin/reports/detail_churchattendace';
	$data['css_cls'] = "info";
	$data['page_title'] = ":: Detail Church Attendance Report ::";
	$data['info_msg'] = "<img src=\"/images/icons/info_small.png\" /> &nbsp; Kindly find below church attendees.";
	$page_res['page_name'] = "<span style='font-weight:bolder;'>Report System</span>";
...
function getDetailedChurchAttendanceReport($view, $data){
	...
	if(misc::required($param)){
		...
		$arrAttendanceInfo = churchmanager::getDetailChurchAttendanceReport($selected_time, $church_id);
	...
	global $page_res;
		...
		switch ($type):
			...
			case 'summarychurchattendance':
				...
				$view = 'church_admin/reports/summary_churchattendace_chart';
				$data['css_cls'] = "info";
				$data['page_title'] = ":: Summary Church Attendance Chart Report ::";
				$data['info_msg'] = "<img src=\"/images/icons/info_small.png\" /> &nbsp; Please select a Church Service Date.";
				$page_res['page_name'] = "<span style='font-weight:bolder;'>Report System</span>";
				...
				/*if(isset($_POST['cmdgo'])){
					...
					$data['posted'] = true;
					$this->getChurchAttendaceSummaryChatReport();
...
function getChurchAttendaceSummaryChatReport(){
	
/*	global $page_res;
	$this->general_page_resource();*/
	
	echo "God is a healing God."; exit;
	
	
	$param = strip_tags($_POST['service_date']);
	
	echo "success|$param"; exit;
	...
	if(misc::required($param)){
		...
		echo "success|$service_time"; exit;
		
		$this->session->set_userdata(array('service_time'=>$service_time));

		$church_id = $page_res['church_id'];

		
		$arrAttendanceInfo = churchmanager::getSummaryChurchAttendanceReport($service_time, $church_id);
		$data['nattendees'] = count($arrAttendanceInfo['id']);
		
		//ret the number of church members in this church
		$data['totalmembers'] = useraccount::loadTotalRefRecord($where=array('church_id'=>$church_id), $fld='church_id', $tblname='tbl_users');

		$data['css_cls'] = "info";
		$data['page_title'] = ":: Detailed Church Attendance Chart Report ::";
		$data['info_msg'] = "<img src=\"/images/icons/info_small.png\" /> &nbsp; Please select a Church Service Date.";
		$page_res['page_name'] = "<span style='font-weight:bolder;'>Report System</span>";


		$msg = "success|$arrAttendanceInfo[id][0]/$arrAttendanceInfo[church_name][0]/$arrAttendanceInfo[total_attendance][0]/$arrAttendanceInfo[service_time][0]/$arrAttendanceInfo[service_date][0]/$data[nattendees]/$data[totalmembers]";
		echo $msg; exit;
