...
class churchmember extends CI_Controller {
	...
	function my_profile(){
		...
		$online_users = useraccount::loadDetails($tableName="tbl_users",$arrFilter=array('church_id'=>$this->session->userdata('church_id'), 'is_online'=>1),$arrAttribute=array('id', 'church_id', 'first_name', 'last_name', 'is_online'),$num=NULL,$orderBy='');
		
		
		
		#load the view
		//$this->load->view('vchurch/v1/church_member/vw_dashboard',array('payment_method'=>$payment_method, 'service_detail'=>$service_detail, 'giving'=>$giving, 'notice_board'=>$notice_board,'church_detail'=>$church_detail, 'support'=>$support, 'data'=>$data, 'user_detail'=>$user_detail, 'page_res'=>$page_res, 'comment'=>$comment, 'testimony'=>$testimony, 'celebrant'=>$celebrants, 'testimonyInfo'=>$testimonies, 'announcement'=>$announcement, 'onlineuser'=>$online_users));
		
		$this->load->view('church_member/vw_index',array('payment_method'=>$payment_method, 'service_detail'=>$service_detail, 'giving'=>$giving, 'notice_board'=>$notice_board,'church_detail'=>$church_detail, 'support'=>$support, 'data'=>$data, 'user_detail'=>$user_detail, 'page_res'=>$page_res, 'comment'=>$comment, 'testimony'=>$testimony, 'celebrant'=>$celebrants, 'testimonyInfo'=>$testimonies, 'announcement'=>$announcement, 'onlineuser'=>$online_users));
...
function edit_profile(){
	...
	$user_detail = $this->useraccount->loadDetails($tblname="tbl_users",$arrFilter=array('user_name'=>$this->misc->cleanUserName($this->session->userdata('user_name'))),$arrAttribute=array('user_name','id','church_id', 'access_level_id', 'email', 'first_name', 'last_name', 'profile_pic', 'country'),$num=1,$orderBy='');
		...
		$data['firstname'] = $this->session->userdata('first_name');
		$data['lastname'] = $this->session->userdata('last_name');
		$data['title'] = "*Editing Profile - ".$data['firstname']." ".$data['lastname']." - Christ Embassy Virtual Church";
		
		//$this->load->view('church_member/edit_profile', $data);
		$this->load->view('may2014/edit_profile', $data);
...
function edit_profile_picture(){
	...
	$this->load->view('church_member/edit_profile_picture', $data);
...
function change_password(){
	...
	$page_res['user_name'] = $this->session->userdata('user_name');
	...
	// get the view content
		...
		$data['firstname'] = $this->session->userdata('first_name');
		$data['lastname'] = $this->session->userdata('last_name');
		$data['title'] = "*Changing Password - ".$data['firstname']." ".$data['lastname']." - Christ Embassy Virtual Church";
		
		$this->load->view('may2014/change_pwd', $data);
}//end function

function edit_coveritlive(){
	$this->load->model('querymanager');
	if(!($this->session->userdata('user_name')=="Iman001"&&$this->session->userdata('email')=="Iman.asangansi@gmail.com")){
	//if(FALSE){
		$here = $_SERVER['REQUEST_URI'];
		header('Location: /auth/login?ref='.urlencode($here));
		exit;
	...
	if($this->input->post()){
		...
		$iframe = mysql_real_escape_string($this->input->post('iframe'));
		$id = $this->input->post('id');
		$sql = "UPDATE `coveritlive` SET `header`='{$header}', `iframe`='{$iframe}' WHERE `id`='{$id}'";
		$update = $this->querymanager->query($sql);
		if($update!=false){
			redirect("churchmember/edit_coveritlive");
		}
	}
	$query = $this->querymanager->query("SELECT * FROM `coveritlive` LIMIT 1");
	$data['coveritlive'] = mysql_fetch_assoc($query);
	$this->load->view('church_member/zzcover', $data);
}
...
function church_service(){
	...
	#lets obtain the church detail for this member/guest
	$church_detail = useraccount::loadDetails($tblname="tbl_churches",array('id'=>$this->session->userdata('church_id')),$arrAttribute=array('id','church_name','stream_url','ipad','blackberry','android','status','file_stream'),$num=1,$orderBy='');
	$data['church'] = $church_detail[0];
	$data['cover'] = useraccount::loadDetails($tblname="coveritlive",$arrFilter=NULL,$arrAttribute=array('id','header','iframe'),$num=1, $orderBy=array('id'=>'DESC'));
	//die(var_dump($data['church']));
	$this->load->view('live', $data);
}

function church_service1(){
	$this->load->model('querymanager');
	if($this->session->userdata('user_name')==""){
		$here = $_SERVER['REQUEST_URI'];
		header('Location: '.base_url().'auth/login?ref='.urlencode($here));
	...
	$query = $this->querymanager->query("SELECT * FROM `coveritlive` LIMIT 1");
	$coverit = mysql_fetch_assoc($query);
...
$view = "church_member/nu_church_service";
$load = array('payment_method'=>$payment_method, 'service_detail'=>$service_detail, 'giving'=>$giving, 'notice_board'=>$notice_board,'church_detail'=>$church_detail, 'support'=>$support, 'data'=>$data, 'user_detail'=>$user_detail, 'page_res'=>$page_res, 'comment'=>$comment, 'church_service'=>$church_service);
$load['title'] = "Live Service - Chirst Embassy Virtual Church";
$load['coveritlive'] = $coverit;

$this->load->view($view, $load);
...
function send_salvation_call(){
	...
	sessiondata::general_page_resource();
		...
		//get the user inputs
		$fname = validator_lib::cleanUserName($_POST['fname']);
		$lname = validator_lib::cleanUserName($_POST['lname']);

		$name = $fname." ".$lname;
		$scallid = $this->misc->genRand(mt_rand(5,10));
		$scalltime = time();
		
		$detail = array(
					'user_id'=>intval($this->session->userdata('user_id')),
					'invite_code'=>$scallid,
					'invite_name'=>strip_tags($name),
					'invite_email'=>validator_lib::cleanEmail($_POST['email']),
					'invite_country'=>strip_tags($_POST['country']),
					'salvation_call_url'=>'http://vchurch.christembassy.org/salvationcall/index/'.$scallid."?acb=me",
					'accept_call'=>0,
					'invitation_cat_id'=>2,
					'time_posted'=>time(),
					'status'=>0);
		...
		if(count($error)==0):  //if there are no errors in the captured inputs
			...
			if($return_flag == 1){	
				...
				if($flag_mail_sent['status']==true):
					util_lib::display_message(array('A call to salvation mail has been sent to the recipient.'), $msg_type='success', $img_source='/images/icons/success_small.png');	
				endif;
...
function feedback(){
	...
	$data['css_cls'] = 'info';
	//$data['frmnote_msg'] = '<img src="/images/icons/info_small.png" align="absmiddle" />&nbsp;To take note, kindly fill out the below form.';
	$data['info_msg'] = '<img src="/images/icons/info_small.png" align="absmiddle" />&nbsp;Send us feedback.';
	...
	$data['title'] = "Feedback - Christ Embassy Virtual Church";
...
function send_tract(){
	...
	$data['title'] = "Send A Tract - Christ Embassy Virtual Church";
...
function send_tract_preview(){
	...
	$data['title'] = "Send Tract - Christ Embassy Virtual Church";
	
	$this->load->view('may2014/send_tract_preview', $data);
}
...
function send_this_card(){
	...
	$imgsrc = "http://vchurch.christembassy.org".$imgsrc;
...
function videos(){
	if($this->session->userdata('user_name')==""){
		$here = $_SERVER['REQUEST_URI'];
		$this->session->set_flashdata('error', 'Log in to continue');
		redirect(base_url().'auth/login?ref='.urlencode($here));
	}
...
function videos1(){
	...
	$view = "church_member/videos/nu_video_index";
	//$view = "may2014/service_highlights";
	/////////////////////////////////////
	$data['flag_msg_status'] = 'info';
	$data['css_cls'] = 'info';
	//$data['frmnote_msg'] = '<img src="/images/icons/info_small.png" align="absmiddle" />&nbsp;To take note, kindly fill out the below form.';
	...
	$page_res['page_name'] = '<strong>Live Service</strong>.';
	...
	$allvideos = useraccount::loadDetails($tblname="tbl_videos",$arrFilter=array('church_id'=>intval($page_res['church_id']),'approved'=>1),$arrAttribute=array('id','video_code','church_id', 'video_title','video_desc' , 'video_url', 'video_thumbnail_url' , 'video_category', 'approved', 'time_posted'),$num=NULL, $orderBy=array('id'=>'DESC'));
	...
	if($video_code > 0){
		...
		$defvideo = useraccount::loadDetails($tblname="tbl_videos",$arrFilter=array('approved'=>1),$arrAttribute=array('id','video_code','church_id', 'video_title','video_desc' , 'video_url', 'video_thumbnail_url' , 'video_category', 'approved', 'time_posted'),$num=1,$orderBy=array('id'=>'DESC'));
...
function featuredtestimony(){

	global $page_res;
	sessiondata::general_page_resource();
	$postID = intval($this->uri->segment(3));
	
	//retrive the testimony
	
	

}//end function

function salvationcall(){
	if($this->session->userdata('user_name')==""){
		$here = $_SERVER['REQUEST_URI'];
		header('Location: /auth/login?ref='.urlencode($here));
		exit;
	...
	$data['info_msg'] = '<img src="/images/icons/info_small.png" align="absmiddle" />&nbsp;Kindly fill out the detail on the below form to send a salvation call to your loved ones; and save a soul.';
	
	$data['page_title'] = 'Salvation Call | Christ Embassy Virtual Church.';
	$page_res['page_name'] = '<strong>Salvation Call</strong>.';
	...
	//$view = "church_member/salvationcall";	
	$view = "may2014/invite";	
	
	$this->load->view($view, array('church_detail'=>$church_detail, 'data'=>$data, 'page_res'=>$page_res, 'notice_board'=>$notice_board, 'support'=>$support, 'soulswon'=>$soulswon, 'invites'=>$invites, 'title'=>"Invite A Friend - Christ Embassy Virtual Church"));
...
///////////////////////////////////////////////////////////////////////////////////
function sendsalvationcall(){
	
	$fname=validator_lib::cleanUserName($_POST['fname']);
	$lname=validator_lib::cleanUserName($_POST['lname']);
	
	$detail = array('user_id'=>$this->session->userdata('user_id'),
					'invite_name'=>validator_lib::cleanUserName($_POST['fname']),
					'invite_email'=>validator_lib::cleanEmail($_POST['email']),
					'invite_country'=>strip_tags($_POST['country']),
					'salvation_call_url'=>SALVATION_CALL_URL,
					'accept_call'=>0,
					'invitation_cat_id'=>3,   //church user or member
					'status'=>0,
					'time_posted'=>time());
